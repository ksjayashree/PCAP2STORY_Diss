...
from capture_normal_baseline import cleanup_synthetic_entries, withdraw_churn_event, get_next_n_id, save_next_n_id, get_incremented_n_id, COUNTER_FILE, PCAPS
...
def configure_pe_silence_state(silent_pes):
    ...
    for pe_idx in range(1, 6):
        container = f"clab-pcap2story-3rr-dev-xpe{pe_idx}"
...
def run_churn_event_active_pes(active_pes, pe_rotation_idx, n_id, active_timers):
    ...
    pe_name = f"xpe{pe_idx}"
    container = f"clab-pcap2story-3rr-dev-{pe_name}"
...
def capture_normal_silent_pe(silent_pes, load="moderate", duration_minutes=2):
    ...
    target_dir = os.path.join(PCAPS, scenario_name)
