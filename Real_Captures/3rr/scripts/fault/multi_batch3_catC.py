"""
Batch 3, 3rr: 3 Category C pairs: link_down + mac_mobility, rt_misconfig + rd_collision,
pe_cease + rd_collision.
"""
...
def ensure_healthy():
    return AS.ensure_healthy("multi_batch3_catC_3rr")


def gen_catC_linkdown_macmobility():
    name = "catC_linkdown_xpe1_macmobility_xpe4to5"
    ...
    time.sleep(8)
    t1 = AS.now_iso()
    AS.link_down_bfd_inject("xpe1")
    time.sleep(10)
    AS.link_down_bfd_recover("xpe1")
    tr1 = AS.now_iso()
    time.sleep(5)
    t2 = AS.now_iso()
    add_mac("xpe4", MAC, IP); time.sleep(4)
    del_mac("xpe4", MAC, IP); add_mac("xpe5", MAC, IP); time.sleep(6)
    origin_w = not is_local("xpe4", MAC); dest_p = is_present("xpe5", MAC)
    time.sleep(5)
    stop_and_collect_captures(name, target_dir)
    del_mac("xpe5", MAC, IP)
    meta = {"multi_incident": True, "category": "C", "incidents": [
        {"event_affected_node": "XPE1", "fault_type": "Link Down", "trigger_mechanism": "BFD Down",
        ...
        {"event_affected_node": "XPE4", "fault_type": "mac_mobility", "mechanism": "clean_move",
         "origin_pe": "XPE4", "destination_pe": "XPE5", "test_mac": MAC, "test_ip": IP,
         "time_of_move": t2, "origin_route_withdrawn": origin_w, "route_transferred": origin_w and dest_p},
    ], "causal_relationship": "none -- XPE1 standalone (no ESI) vs mac move on xpe4/xpe5 ES pair, independent domains"}
...
def gen_catC_rtmisconfig_rdcollision():
    name = "catC_rtmisconfig_xpe6_rdcollision_xpe1xpe10"
    ...
    time.sleep(8)
    t1 = AS.now_iso()
    AS.rt_import_only_inject("xpe6")
    time.sleep(10)
    t2 = AS.now_iso()
    apply_rd("xpe1", RD); apply_rd("xpe10", RD)
    time.sleep(16)
    stop_and_collect_captures(name, target_dir)
    AS.rt_import_only_recover("xpe6")
    revert_rd("xpe1", RD); revert_rd("xpe10", RD)
    meta = {"multi_incident": True, "category": "C", "incidents": [
        {"event_affected_node": "XPE6", "fault_type": "RT Misconfiguration", "trigger_mechanism": "Plain Import/Export Mismatch",
        ...
        {"event_affected_nodes": ["XPE1", "XPE10"], "fault_type": "RT Misconfiguration", "fault_subtype": "RD Collision",
    ...
    ], "causal_relationship": "none -- XPE6 (ES pair member, xrr2 domain) vs RD collision on xpe1/xpe10 (both standalone, different RRs)"}
    write_meta(target_dir, meta)
    RESULTS.append({"name": name, "status": "OK"})
    print(f"[DONE] {name}")
...
def gen_catC_pecease_rdcollision():
    name = "catC_pecease_xpe2_rdcollision_xpe8xpe9"
    ...
    meta = {"multi_incident": True, "category": "C", "incidents": [
        ...
        {"event_affected_nodes": ["XPE8", "XPE9"], "fault_type": "RT Misconfiguration", "fault_subtype": "RD Collision",
         "trigger_mechanism": "Shared Route Distinguisher (RD Collision)", "colliding_rd": RD,
         "time_of_first_fault": t2, "recovered": False, "time_of_recovery": None},
    ], "causal_relationship": "none -- XPE2 (xrr1 domain) vs RD collision on xpe8/xpe9 (xrr3 domain)"}
    write_meta(target_dir, meta)
    RESULTS.append({"name": name, "status": "OK"})
    print(f"[DONE] {name}")
