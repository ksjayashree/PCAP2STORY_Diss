...
for idx in range(1, 11):
    container = f"clab-pcap2story-3rr-dev-xpe{idx}"
    lo_ip = f"10.0.0.{10 + idx}"
...
subprocess.run(["wsl", "docker", "exec", "clab-pcap2story-3rr-dev-xrr1", "vtysh", "-b"], check=False)
subprocess.run(["wsl", "docker", "exec", "clab-pcap2story-3rr-dev-xrr2", "vtysh", "-b"], check=False)
